...
export function CFItemsTable() {
  ...
  useEffect(() => {
    const fetchCFItems = async () => {
      if (selectedDocId) {
        ...
        setSelectedCFItems(items);
      }
    };

    fetchCFItems();
  }, [selectedDocId, firestore]);
  ...
  const saveChanges = async () => {
    ...
    try {
      const docRef = doc(firestore, "json", selectedDocId);
      ...
      <Table striped bordered hover>
        <thead>
          <tr>
            <th onClick={() => handleSort("humanCodingScheme")}>
              Human Coding Scheme {renderSortIndicator("humanCodingScheme")}
            </th>
            <th onClick={() => handleSort("fullStatement")}>
              Full Statement {renderSortIndicator("fullStatement")}
            </th>
            <th>CFItemType</th>
        ...
        <tbody>
          {selectedCFItems.map((item, index) => (
            <tr key={index}>
              <td>{renderEditableCell(item, "humanCodingScheme")}</td>
              <td>{renderEditableCell(item, "fullStatement")}</td>
              <td>{item.CFItemType}</td>
              <td>{item.educationLevel.join(", ")}</td>
